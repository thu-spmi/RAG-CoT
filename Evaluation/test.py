...
def process_batch(batch, tokenizer, model, prompt_template, max_new_tokens=256, repetition_penalty=1.1, source_max_len=256):
    ...
    for i, record in enumerate(batch):
        ...
        input_string = prompt_template.format_map({'question': question})
        sentences.append(input_string)
    start = time.time()
    contextualized_inputs = tokenizer(sentences, return_tensors="pt", padding=True, truncation = True, max_length = source_max_len).to(model.device)
...
if __name__ == '__main__':
    ...
    if args.prompt_template == 'llama2':
        prompt_template = '[INST]Give a short answer to the Question, no extra details and expalanations. \nQuestion: {question}\n[/INST]'
