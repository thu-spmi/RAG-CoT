    ...
    def __init__(self, pathname, tokenizer, args, eval_mode=False):
        ...
        with open(pathname, 'r') as f:
            ...
            for ln in tqdm(f):
                ...
                # 构建输入和目标文本
                source_text = self.tokenizer.bos_token + self.instruction_template.format_map({'question': question})
                ...
                labels = torch.tensor([IGNORE_INDEX for _ in range(len(tokenized_source['input_ids']))] + tokenized_target['input_ids'])
